...
DBT_PROJECT_DIR = "~/dbt_/traffic_flow"
DBT_PROFILE_DIR = " ~/OneDrive/Documents/GitHub/Data-warehouse-tech-stack-with-posgreSQL-DBT-Airflow/dbt_/.dbt"
extract = Extractor()
loader = Loader()
...
with DAG(dag_id="workflow",default_args=default_args,schedule_interval='@daily', catchup=False) as dag:
    ...
    dbt_run = BashOperator(
        task_id="dbt_run",
        bash_command=f"cd ~/dbt_/traffic_flow && ~/local/dbt run --profiles-dir {DBT_PROFILE_DIR}"
    )    
    dbt_test = BashOperator(
        task_id="dbt_test",
        bash_command=f"cd ~/OneDrive/Documents/GitHub/Data-warehouse-tech-stack-with-posgreSQL-DBT-Airflow/dbt_/traffic_flow run --profiles-dir {DBT_PROFILE_DIR}",
    )
